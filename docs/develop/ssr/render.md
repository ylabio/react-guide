...
При повторном рендере хук `useInit()` на сервере уже не исполняется, так как инициализация была выполнена при первом рендере.
В хуке учитывается признак `SSR.firstRender === true`. На клиенте используется тот же хук `useInit()`, но там, наоборот — 
не исполняется при первом рендере, так как состояние считается уже инициализироанным из-за рендера на сервере. (*Если SSR 
не активен, то на клиенте `useInit()` при первом рендере работает*)
...
После первого рендера признак `SSR.firstRender` сбрасывается, чтобы при переходах на другие страницы приложения 
...
Если контейнер не рендерился на сервере, но ему нужно выполнять хук `useInit()` при первом рендере на клиенте, то необходимо 
передать в `useInit()` четвертый параметр `force` равный `true`. Тогда хук будет работать и при первом рендере.
