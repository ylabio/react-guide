...
1. Вёрстка (теги html) только в [компонентах](/docs/check/component.md) в директории `@src/components/`, в других частях приложения вёрстка недопустима.
...
8. Для подставления модификатора в атрибуте `className` лучше применить функцию [`@src/utils/themes`](https://github.com/ylabio/react-skeleton/blob/master/src/utils/themes.js)
...
10. Стилизация на jss, styled-components и подобных не используются. 
    ...
    - 👎 Нетиповые задачи вёрстки усложняются из-за специфики инструмента. Специфика проявляется, так как 
         инструмент задаёт свои правила (формат, атомарность...), а не дополняет css. 
    - Какую проблему и какой ценой решают? Есть аргументы — обсуди с командой.      

11. Для разметки областей создавать компоненты Layout* в `@src/components/layout/`. 
...
14. Общие стили, шрифты, css переменные, нормалайзеры, картинки и другие ресурсы разметки помещаются в директории `@src/theme/`
